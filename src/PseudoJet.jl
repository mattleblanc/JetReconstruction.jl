...
"""
function PseudoJet(px::Real, py::Real, pz::Real, E::Real; cluster_hist_index::Int = 0, _pure_ghost = false)
    ...
    PseudoJet(px, py, pz, E, cluster_hist_index, pt2, inv_pt2, rap, phi, _pure_ghost)
end
...
function PseudoJet(; pt::Real, rap::Real, phi::Real, m::Real = 0,
                   cluster_hist_index::Int = 0, _pure_ghost = false)
    ...
    PseudoJet(px, py, pz, E, cluster_hist_index, pt * pt, 1 / (pt * pt), rap, phi, _pure_ghost)
end
...
"""
function PseudoJet(jet::LorentzVector; cluster_hist_index::Int = 0, _pure_ghost = false)
    PseudoJet(jet.x, jet.y, jet.z, jet.t; cluster_hist_index = cluster_hist_index, _pure_ghost)
end
...
"""
function PseudoJet(jet::LorentzVectorCyl; cluster_hist_index::Int = 0, _pure_ghost = false)
    PseudoJet(; pt = pt(jet), rap = rapidity(jet), phi = phi(jet), m = mass(jet),
              cluster_hist_index = cluster_hist_index, _pure_ghost = _pure_ghost)
end
...
value.
"""
function PseudoJet(jet::Any; cluster_hist_index::Int = 0, _pure_ghost = false)
